...
class ITNews:
    ...
    def getHeadLink(self):
        ...
        # https: // news.naver.com / main / clusterArticles.nhn?id = c_202006121340_00000002 & mode = LSD & mid = shm & sid1 = 105
        return results
        #print(results[seed]) #헤드라인 주소
...
class windows(QDialog,ui):
    ...
    def MvAreaOpen(self):
        ...
        def loadImageFromWeb_01(self):
            ...
            self.qPixmapWebVar = self.qPixmapWebVar.scaledToWidth(200)
            self.lbl_movie_image_1.setPixmap(self.qPixmapWebVar)
        ...
        def loadImageFromWeb_02(self):
            ...
            self.qPixmapWebVar.loadFromData(imageFromWeb)
            self.qPixmapWebVar = self.qPixmapWebVar.scaledToWidth(200)
            self.lbl_movie_image_2.setPixmap(self.qPixmapWebVar)
        ...
        def loadImageFromWeb_03(self):
            ...
            self.qPixmapWebVar.loadFromData(imageFromWeb)
            self.qPixmapWebVar = self.qPixmapWebVar.scaledToWidth(200)
            self.lbl_movie_image_3.setPixmap(self.qPixmapWebVar)
        ...
        def loadImageFromWeb_04(self):
            ...
            self.qPixmapWebVar.loadFromData(imageFromWeb)
            self.qPixmapWebVar = self.qPixmapWebVar.scaledToWidth(200)
            self.lbl_movie_image_4.setPixmap(self.qPixmapWebVar)
        ...
        def loadImageFromWeb_05(self):
            ...
            self.qPixmapWebVar.loadFromData(imageFromWeb)
            self.qPixmapWebVar = self.qPixmapWebVar.scaledToWidth(200)
            self.lbl_movie_image_5.setPixmap(self.qPixmapWebVar)
    ...
    def MealAreaOpen(self):
        ...
        for i in range(len(list)):
            list[0] = meals_date[1].get_text() + '\n' + meals_meal[7].get_text('\n') + '\n'
            list[1] = meals_date[2].get_text() + '\n' + meals_meal[8].get_text('\n') + '\n'
            list[2] = meals_date[3].get_text() + '\n' + meals_meal[9].get_text('\n') + '\n'
            list[3] = meals_date[4].get_text() + '\n' + meals_meal[10].get_text('\n') + '\n'
            list[4] = meals_date[5].get_text() + '\n' + meals_meal[11].get_text('\n') + '\n'
        meal_table = list[0] + '\n' + list[1] + '\n' + list[2] + '\n' + list[3] + '\n' + list[4]
        self.lbl_school_meal_deajon.setText(meal_table)
        ...
        meals_02 = soup.find(class_='basic_table center')
        meals_date02 = meals_02.find_all('th')
        meals_meal02 = meals_02.find_all('td')
        driver.close()

        list_02 = [0, 1, 2, 3, 4]

        for i in range(len(list)):
            list_02[0] = meals_date02[1].get_text() + '\n' + meals_meal02[7].get_text('\n') + '\n'
            list_02[1] = meals_date02[2].get_text() + '\n' + meals_meal02[8].get_text('\n') + '\n'
            list_02[2] = meals_date02[3].get_text() + '\n' + meals_meal02[9].get_text('\n') + '\n'
            list_02[3] = meals_date02[4].get_text() + '\n' + meals_meal02[10].get_text('\n') + '\n'
            list_02[4] = meals_date02[5].get_text() + '\n' + meals_meal02[11].get_text('\n') + '\n'
        meal_table = list_02[0] + '\n' + list_02[1] + '\n' + list_02[2] + '\n' + list_02[3] + '\n' + list_02[4]
        self.lbl_school_meal_nonsan.setText(meal_table)
...
class Con_Area(QDialog,uiCon):              #### 위 아래 순서 꼭 지킬 것!!!!!!
    def __init__(self,parent):
    ...
    #드루와
        ...
        ## 주요 민원부서 전화번호                 #### 위 아래 순서 바꿀 때 꼭 잘 확인하고 바꿔요!!!!!
